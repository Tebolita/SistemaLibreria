...
export default function CartPopover({ onClose }: Props) {
  ...
  // 🧩 Cargar carrito continuamente mientras esté visible
  useEffect(() => {
  ...
  const cambiar = (id: number, delta: number) => {
    const nuevo = carrito.map((p) => {
      const productoId = p.idProducto || p.IdProducto;
      ...
      {carrito.length === 0 ? (
        ...
        <>
          <div className="max-h-64 overflow-y-auto pr-1 space-y-2">
            {carrito.map((p, index) => {
              const productoId = p.idProducto || p.IdProducto || index;
              const nombre = p.nombre || p.Nombre || "Producto sin nombre";
              const imagen = p.imagen || p.Imagen || "/placeholder.png";
              const precioNum = Number(p.precio || p.Precio || 0);
